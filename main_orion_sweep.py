...
base_config = {
    ...
    "low_fi_data_folder": "Data/orion_low_fi_data_200",
    "dimension": 2,
    "lhs_sample": 500000,
    "num_epochs": 50000,
    "output_dim": 6,
...
for i, combination in enumerate(sampled_combinations):
    ...
    config["project_name"] = f"orion_hyperparam_sweep_{i}"

    config_filename = f"configs/config_orion_hyperparam_sweep_{i}.yaml"
    with open(config_filename, "w") as f:
        yaml.dump(config, f)

if __name__ == "__main__":
    # for i in range(len(sampled_combinations)):
    config_path = f"configs/config_orion_hyperparam_sweep_{0}.yaml"
